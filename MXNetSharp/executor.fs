...
type Executor(handle : CApi.ExecutorHandle) =   
    new(symbol : Symbol, context, inArgs, argGrad, gradReqType, auxStates) = 
        let inArgs = inArgs |> Seq.map (fun (x : NDArray) -> x.NDArrayHandle) |> Seq.toArray
        let argGrad = argGrad |> Seq.map (fun (x : NDArray) -> x.NDArrayHandle) |> Seq.toArray
        let gradReqType = gradReqType |> Seq.map (fun (x : OpReqType) -> uint32 x) |> Seq.toArray
        //let h = MXExecutor.bind symbol.SymbolHandle (int context.DeviceType) context.DeviceId inArgs argGrad gradReqType auxStates
