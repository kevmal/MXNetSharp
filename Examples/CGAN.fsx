...
let layers : Symbol list = 
    [
        xIn
        for h in [256; 512; 1024] do 
            new FullyConnected(h)
            new LeakyReLU(slope = 0.2)
            new BatchNorm(momentum = 0.8)
        new FullyConnected(28*28)
        new Reshape(shape = [28;28])
    ]


let rec str (x : Symbol) : string = 
    let name = x.GetType().Name
    match x with 
    | :? SymbolOperator as s -> 
        s.OperatorArguments
        |> Seq.choose
            (fun (name, arg) ->
                match arg with 
                | Input (:? ImplicitVariable as v) -> Some(name,"_")
                | Parameter(None) -> None
                | Parameter(Some o) -> Some(name, string o)
                | VarArg(_,s) -> Some(name, (s |> Seq.map str |> String.concat "; " |> sprintf "[%s]"))
                | Input (v) -> Some(name,str v)
            )
        |> Seq.map (fun (name,v) -> if v = "_" then sprintf "_%s" name else sprintf "%s = %s" name v)
        |> String.concat ", "
        |> sprintf "%s(%s)" name
    | _ -> name
        
layers |> Seq.iter (str >> printfn "%s")
