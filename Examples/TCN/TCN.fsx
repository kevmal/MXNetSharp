...
let temporalBlock name inCount outputCount kernelSize stride dilation padding dropout (x : Symbol) = 
    printfn "%A" (kernelSize, stride, padding, dilation)
    let conv1 = Convolution(x, 
                            ...
                            noBias = false,
                            Name = name + "_conv1")
    let conv1Sliced = Slice((conv1), [None;None;None], [None; None; Some -padding], [None;None;None])
    ...
    let conv2 = Convolution(dropout1,
                            ...
                            noBias = false,
                            Name = name + "_conv2")
    let conv2Sliced = Slice((conv2), [None;None;None], [None; None; Some -padding], [None;None;None])
...
let mutable beta1 = 0.9
let mutable beta2 = 0.999
let mutable count = 1
let mutable clr = lr
let preOpt() = 
    ...
    clr <- lr*(sqrt(coef2)/coef1)
    count <- count + 1
let optUpdate (e : Executor) = 
    let mutable epsilon = 1e-8
        ...
        |> Seq.iter
            (fun a ->
                ...
                | ArgBinding ({Name = name; OpReqType = Some WriteTo; Grad = Some grad; NDArray = Some weight}) -> 
                    ...
                    MX.AdamUpdate([weight], weight, grad, m, v, clr, beta1, beta2, epsilon)
                | _ -> ()
            )
